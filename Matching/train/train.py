...
def argument_parser():
    ...
    parser.add_argument('--template', type=str, default='이 뉴스의 주제는 {}이다.',
                        help='Template for label'
                       )  
...
def train(model, train_dataloader, optimizer, scheduler, args, scaler):
    ...
    for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
        ...
        entailment = batch['entailment'].to(args.device)

        optimizer.zero_grad()

        if args.amp:
            with autocast():
                train_loss = model(input_ids, attention_mask, token_type_ids, entailment)
        else:
            train_loss = model(input_ids, attention_mask, token_type_ids, entailment)
...
def valid(model, valid_dataloader, args):
    ...
    for _, batch in enumerate(valid_dataloader):
        ...
        with torch.no_grad():
            valid_loss = model(input_ids, attention_mask, token_type_ids, entailment)
...
def main(args):
    ...
    num_labels = len(dataset.label_text.keys())
    
    if args.device == 'cuda':
        model = EntailmentModel(args).to(args.device)
   ...
   # Save Best Model
    ...
    accuracy = test(best_model, test_dataloader, num_labels, args.device)
