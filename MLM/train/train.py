...
def argument_parser():
    ...
    parser.add_argument('--pattern', type=str, default='[MASK]:{}',
                        help='Pattern of input data. It must have one [MASK] and {}'
                       )
...
def valid(model, valid_dataloader, args):
    ...
    for _, batch in enumerate(valid_dataloader):
        ...
        with torch.no_grad():
            valid_loss = model(input_ids, attention_mask, token_type_ids, label)
...
def main(args):
   ...
   # Save Best Model
    ...
    accuracy = test(best_model, test_dataloader, args)
